...
declare -A VENV_CHROOT_COPY_BIN=(
    ...
    [`which g++`]="$VENV_ROOT_DIR/cpp/bin/g++"
    [`which make`]="$VENV_ROOT_DIR/cpp/bin/make"
    [`which git`]="$VENV_ROOT_DIR/cpp/bin/git"
)
declare -A VENV_CHROOT_ENV=(
    ...
    [HOME]=$VENV_ROOT_DIR
    [PWD]=$VENV_ROOT_DIR
    [PATH]="${VENV_ROOT_DIR}/cpp/bin:${VENV_ROOT_DIR}/bin:$PATH"
...
main() {
    ...
    for key in "${!VENV_CHROOT_COPY_BIN[@]}"; do
        ...
        # Get the directory and file name of the binary file
        bin_dirname="$(dirname $value)"
        ...
        if [[ ! -f $value ]]; then
            # Does not exists: Copy
            printf "COPY %s => %s\n" $key $value
            mkdir -pv $bin_dirname && cp -r $key $value && \
                echo -e "Successfully copied $key => $value" || \
                echo -e "Error copying $key => $value"
        fi
    done
